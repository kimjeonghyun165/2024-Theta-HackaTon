    ...
    <ul>
      {sentenceArr.map(([title, content], index) => (
        <li className="flex items-center gap-4 py-1" key={index}>
          ...
          <span className="bg-fifth/[.2] rounded-[30px] px-10 flex items-center py-3 min-w-48 justify-center">
            {content}
          </span>
          {icons[title as IconKeys] && <IconBtn icon={icons[title as IconKeys]} bgColor='bg-fifth/[.2] hover:bg-black'/>}
        </li>
      ))}
    </ul>
  );
